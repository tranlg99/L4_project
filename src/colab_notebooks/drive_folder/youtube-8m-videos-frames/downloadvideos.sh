...
if [ "$1" -eq 0 ]; then
	while read line
		do
			...
			$YTDL -f 18 "http://www.youtube.com/watch?v=$line" -o ./videos/"$name%(title)s-%(id)s.%(ext)s"
	...
	while read line
		do
			if [ "$limit" -gt 0 ]; then
				limit=$(($limit-1))
				$YTDL -f 18 "http://www.youtube.com/watch?v=$line" -o ./videos/"$name%(title)s-%(id)s.%(ext)s" 2>&1 | tee log.txt
